...
class ApiService2 {
    ...
    private static init() {
      ApiService2.instance = axios.create({
        baseURL: ApiService2.baseUrl,
      })
      ApiService2.instance.defaults.headers.common.Accept = 'application/json'
      ApiService2.instance.interceptors.request.use(
        req => {
    ...
    private static async refresh(error: any): Promise<any> {
      ...
      if (this.isRefreshing) {
        return new Promise<any>((resolve, reject) => {
          ApiService2.failedQueue.push({ resolve, reject })
        })
          .then(token => {
      ...
      return new Promise<any>((resolve, reject) => {
        ...
        axios
          .post(`${ApiService2.baseUrl}/auth/refresh`, { refresh_token: JwtService.getRefreshToken() })
          .then(({ data }) => {
            ...
            ApiService2.processQueue(null, data.data.access_token)
            resolve(axios.request(originalRequest)) // başarılı oldu ise orjinal requesti tekrar gönder ve resolve et
          })
          .catch(err => {
            ApiService2.processQueue(err, null) // başarısız oldu ise hepsini reject et ve çıkış yap
            useUserStore().logout()
            reject(err)
          })
          .finally(() => {
            ApiService2.isRefreshing = false
          })
      })
    }
    ...
    private static async request<T>(
      ...
      try {
        const { data } = await ApiService2.instance.request<ApiResponse<T>>(config)
