...
import { g as commonjsGlobal, h as getDefaultExportFromCjs, f as loadDictPresets, c as buildRegex, i as c, d as replace } from './shared/case-police.fef7a942.mjs';
...
const parseIgnore = /*@__PURE__*/getDefaultExportFromCjs(parseGitignore);
...
const isText = /*@__PURE__*/getDefaultExportFromCjs(isTextPath);
...
async function run() {
  ...
  if (existsSync(".gitignore")) {
    const gitignore = await promises.readFile(".gitignore", "utf8");
  ...
  if (argv.dict) {
    const str = await promises.readFile(argv.dict, "utf8");
  ...
  await Promise.all(files.map((file) => limit(async () => {
    ...
    if (replaced) {
      wrote.push(file);
      if (argv.fix)
        await promises.writeFile(file, replaced, "utf-8");
    }
  })));
  if (!wrote.length) {
