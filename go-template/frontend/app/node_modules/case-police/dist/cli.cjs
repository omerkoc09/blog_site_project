...
const utils$l = require('./shared/case-police.f55355bd.cjs');
...
Object.defineProperty(patterns$1, "__esModule", { value: true });
patterns$1.removeDuplicateSlashes = patterns$1.transform = void 0;
/**
...
patterns$1.transform = transform;
/**
...
patterns$1.removeDuplicateSlashes = removeDuplicateSlashes;
...
var queueMicrotask_1 = typeof queueMicrotask === 'function'
  ? queueMicrotask.bind(typeof window !== 'undefined' ? window : utils$l.commonjsGlobal)
...
const patternManager = patterns$1;
...
const fg = /*@__PURE__*/utils$l.getDefaultExportFromCjs(out);
...
const parseIgnore = /*@__PURE__*/utils$l.getDefaultExportFromCjs(parseGitignore);

var textExtensions$2 = [
	"ada",
	"adb",
	"ads",
	"applescript",
...
var textExtensions$1 = textExtensions$2;

const path = require$$0__default;
const textExtensions = textExtensions$1;

const extensions = new Set(textExtensions);

var isTextPath = filePath => extensions.has(path.extname(filePath).slice(1).toLowerCase());

const isText = /*@__PURE__*/utils$l.getDefaultExportFromCjs(isTextPath);
...
const minimist$1 = /*@__PURE__*/utils$l.getDefaultExportFromCjs(minimist);
...
async function run() {
  ...
  if (node_fs.existsSync(".gitignore")) {
    const gitignore = await node_fs.promises.readFile(".gitignore", "utf8");
    ignore.push(...parseIgnore(gitignore).patterns);
  }
  if (argv.ignore)
    ignore.push(...argv.ignore.split(",").map((i) => i.trim()));
  ignore = ignore.filter(Boolean);
  let dict = {};
  if (!argv["no-default"])
    dict = await utils$l.loadDictPresets(argv.preset);
  if (argv.dict) {
    const str = await node_fs.promises.readFile(argv.dict, "utf8");
  ...
  if (!argv._.length)
    argv._.push("**/*.*");
  const files = await fg(argv._, { ignore }).then((files2) => files2.filter((file) => isText(file)));
  const disabled = (argv.disable ? argv.disable.split(",") : []).map((i) => i.trim().toLowerCase());
  const regex = utils$l.buildRegex(dict);
  const limit = pLimit(5);
  console.log();
  console.log(utils$l.c.inverse(utils$l.c.red(" Case ")) + utils$l.c.inverse(utils$l.c.blue(" Police ")) + utils$l.c.dim(` v${version}`));
  console.log();
  console.log(utils$l.c.blue(files.length) + utils$l.c.dim(" files found for checking, ") + utils$l.c.cyan(Object.keys(dict).length) + utils$l.c.dim(" words loaded\n"));
  const wrote = [];
  await Promise.all(files.map((file) => limit(async () => {
    const code = await node_fs.promises.readFile(file, "utf-8");
    const replaced = await utils$l.replace(code, file, dict, regex, disabled);
    if (replaced) {
      wrote.push(file);
      if (argv.fix)
        await node_fs.promises.writeFile(file, replaced, "utf-8");
    }
  })));
  if (!wrote.length) {
    console.log(utils$l.c.green("All good, well done!"));
  } else {
    if (argv.fix) {
      console.log(utils$l.c.green("\nfiles fixed"));
    } else {
      console.log(utils$l.c.dim(`
${wrote.length} files contain case errors`));
      console.log(utils$l.c.dim("run ") + utils$l.c.magenta(utils$l.c.bold("npx case-police --fix")) + utils$l.c.dim(" to fix\n"));
      process.exitCode = 1;
    }
  }
}
